...
#' 
#' @references
#' Updated NCEP ATPIII criteria (Grundy et al., 2005):
...
#' determined in a fail-fast way: if you do not explicitly set \code{is_fasting_blood_sample = TRUE}, the glucose measurement is considered to be
...
#' still considered hypertensive. With blood pressure \code{NA}/90 mmHg without
#' antihypertensive medication, a patient is still considered hypertensive. A
#' patient with blood pressure \code{NA}/\code{NA} mmHg and no antihypertensive medication is
#' considered \code{NA} for the trait hypertension, but if a patient has \code{NA}/\code{NA} mmHg but
#' does use antihypertensive medication, the patient is considered to have
#' hypertension (even though measurements are missing).
#'
#' @param is_female \code{TRUE} if patient is female, \code{FALSE} if patient is a man.
#' @param waist_circumference Waist circumference (cm) (\code{NA} by default).
#' @param systolic_blood_pressure Systolic blood pressure (mmHg) (\code{NA} by default).
#' @param triglycerides Triglycerides in (mmol/l) (\code{NA} by default).
#' @param diastolic_blood_pressure Systolic blood pressure (mmHg) (\code{NA} by default).
#' @param hdl_cholesterol HDL cholesterol (mmol/l) (\code{NA} by default).
#' @param glucose Glucose (mmol/l) (\code{NA} by default).
#' @param is_fasting_blood_sample \code{TRUE} if fasting blood sample, \code{FALSE} (default) if not or
#'   unknown. 
#' @param has_antihypertensive_drug \code{TRUE} if patient is on antihypertensive drug
#'   treatment, otherwise \code{FALSE} (default).
#' @param has_lipid_drug \code{TRUE} if patient is on fibrate, nicotinic acid or
#'   statin, otherwise \code{FALSE} (default).
#' @param has_glucose_drug \code{TRUE} if patient is on drug treatment for elevated
#'   glucose, otherwise \code{FALSE} (default).
#' @return \code{TRUE} if patient has the metabolic syndrome, \code{FALSE} is not, \code{NA} if
#'   indetermined.
#' @export
